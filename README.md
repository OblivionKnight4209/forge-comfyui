...
mkdir -p "$HOME/forge"
cd "$HOME/Downloads"
wget -O Forge-offline.zip https://github.com/OblivionKnight4209/forge-comfyui/releases/download/v240/Forge-offline.zip
...
The script must print **Forge v240**. Top-left corner of the page must say **240**. Hard refresh (`Ctrl+Shift+R`). If it still says 219, you unzipped an old zip.
...
| **Comic** | Layout + ink chips | One page with panels. Short line → beats. Image tab is not used. |
...
Tick the checkpoints you want. Prompt + **locked seed**. Generate queues them one after another. Compare which mix actually draws your scene. Bad mixes you can stop using.
